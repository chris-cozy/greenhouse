...
describe("Greenhouse local store",()=>{
  ...
  it("keeps one-time and repeating reminders explicit, snoozable, and undoable",()=>{const {store}=makeStore();const plant=store.savePlant({name:"Fern",status:"healthy"});const once=store.saveCare(plant.id,{activityType:"misting",guidance:"Check first",cadenceDays:5,reminderEnabled:true,reminderRepeat:false,nextReminderDate:"2026-08-29"});const repeat=store.saveCare(plant.id,{activityType:"watering",guidance:"Water if dry",cadenceDays:14,reminderEnabled:true,reminderRepeat:true,reminderCadenceDays:3,nextReminderDate:"2026-08-28"});
    const completedOnce=store.completeReminder(once.id,"2026-08-29");expect(completedOnce.reminder.reminderEnabled).toBe(false);expect(completedOnce.reminder.nextReminderDate).toBe("");expect(completedOnce.previous).toEqual({reminderEnabled:true,nextReminderDate:"2026-08-29"});
    expect(store.restoreReminder(once.id,completedOnce.previous).nextReminderDate).toBe("2026-08-29");expect(store.snoozeReminder(once.id,3,"2026-08-29").nextReminderDate).toBe("2026-09-01");expect(store.disableReminder(once.id).reminderEnabled).toBe(false);
    const completedRepeat=store.completeReminder(repeat.id,"2026-08-29").reminder;expect(completedRepeat.reminderEnabled).toBe(true);expect(completedRepeat.nextReminderDate).toBe("2026-09-01");expect(completedRepeat.cadenceDays).toBe(14);expect(completedRepeat.reminderCadenceDays).toBe(3);
    expect(store.dashboard().upcomingReminders[0]).toEqual(expect.objectContaining({id:repeat.id,plantName:"Fern",plantSpriteImage:plant.spriteImage}));store.close()});

  it("migrates existing reminder cadence into the explicit repeat schedule",()=>{const {store}=makeStore();const plant=store.savePlant({name:"Legacy reminder"});const care=store.saveCare(plant.id,{activityType:"watering",guidance:"Legacy",cadenceDays:9,reminderEnabled:true,nextReminderDate:"2026-08-30"});const dbPath=store.dbPath;store.close();const db=new Database(dbPath);db.exec("ALTER TABLE care_items DROP COLUMN reminder_repeat; ALTER TABLE care_items DROP COLUMN reminder_cadence_days; DELETE FROM schema_migrations WHERE version=5");db.close();const migrated=new GreenhouseStore(dbPath);expect(migrated.listCare(plant.id).find(item=>item.id===care.id)).toEqual(expect.objectContaining({reminderRepeat:true,reminderCadenceDays:9,nextReminderDate:"2026-08-30"}));migrated.close()});
