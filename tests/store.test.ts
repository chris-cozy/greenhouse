...
describe("Greenhouse local store",()=>{
  ...
  it("builds an ordered garden from living plants and every terrarium",()=>{const {store}=makeStore();const firstTerrarium=store.saveTerrarium({name:"Cloud Garden"});const resident=store.savePlant({name:"Moss",status:"healthy",terrariumId:firstTerrarium.id});const standalone=store.savePlant({name:"Fern",status:"dormant"});store.savePlant({name:"Memorial",status:"deceased"});const archived=store.savePlant({name:"Archived cutting",status:"healthy"});store.archivePlant(archived.id,true);const secondTerrarium=store.saveTerrarium({name:"Desert Bowl"});const dashboard=store.dashboard();expect(dashboard.gardenPlants).toEqual([{id:resident.id,name:"Moss"},{id:standalone.id,name:"Fern"}]);expect(dashboard.gardenTerrariums).toEqual([{id:firstTerrarium.id,name:"Cloud Garden"},{id:secondTerrarium.id,name:"Desert Bowl"}]);expect(dashboard.livingPlants).toBe(2);expect(dashboard.terrariums).toBe(2);store.close()});
