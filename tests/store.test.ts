...
describe("Greenhouse local store",()=>{
  it("keeps reusable species separate from individual plants",()=>{const {store}=makeStore();const species=store.saveSpecies({commonName:"Button fern",scientificName:"Pellaea rotundifolia",humidityRequirements:"Moderate"});const first=store.savePlant({name:"Mosslight",speciesId:species.id,status:"healthy",tags:["fern"]});const second=store.savePlant({name:"Orbit",speciesId:species.id,status:"healthy"});expect(store.getSpecies(species.id)?.plantCount).toBe(2);expect(store.getPlant(first.id)?.speciesScientificName).toBe("Pellaea rotundifolia");expect(store.getPlant(second.id)?.name).toBe("Orbit");store.close()});
